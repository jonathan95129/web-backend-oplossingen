...
De stukken van Hergé maakten deel uit van een speciale verkoop gewijd aan originele platen. Vijfenveertig van de vijftig stukken vonden een nieuwe eigenaar, voor een totaalbedrag van 1,125 miljoen euro. Naast werk van de auteur van Kuifje werden ook stukken van onder meer Franquin, Vandersteen en Francois Schuiten geveild.',
    ...
    if (isset($_GET['id'])) {
        ...
        if (array_key_exists($id, $artikels)) {
            ...
            $artikels = array($artikels[$id]); // 1 array laten zien
        }
    }
 ...
 <html>
     ...
     <body>
        ...
        <?php foreach ($artikels as $key => $value): ?>
            ...
            <div class="<?php echo ($volledigArtikel)? 'artikelVolledig':'artikel' ?>" >  
                <ul>
                    <li><h2><?php echo $value['titel']; ?></h2></li>
                    <li><h4><?php echo $value['datum']; ?></h4></li>                    
                    <li><img src="img/<?php echo $value['afbeelding']; ?>"></li>
                    
                    <li><p>
                        <?php echo ($volledigArtikel)? 
                            // true
                            str_replace("\r\n", "</p><p>", $value['inhoud']): // white space toevoegen in de text 
                            //false
                            str_replace("\r\n", "</p><p>", substr($value['inhoud'],0,50 )). "..." // string beginnen bij 0 en eindigen bij 50
                            
                             ?>
                    </p></li>

                    <?php if(!$volledigArtikel): ?>
                        <li><a href="deel1.php?id=<?php echo $key ?>">Lees meer</a></li>
                    <?php endif ?>
                </ul>
